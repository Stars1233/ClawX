...
- **🎙️ 音声入力（開発者モード）**：設定で**開発者モード**を有効にすると、チャット入力欄のマイクボタンで音声を録音し、設定済みの音声認識サービスで文字起こししてカーソル位置に挿入できます。**モデル → 音声入力** タブで API タイプ（OpenAI Audio Transcriptions または `input_audio` 方式の OpenAI Chat Completions）とプリセット（OpenAI・Groq・SiliconFlow・Alibaba Cloud Model Studio・カスタム）および API キーを設定できます。
  - マイクをクリックすると権限を確認します。拒否されている場合はダイアログから明示的にシステム設定を開けます。ポリシーによる制限は管理者への問い合わせが必要な場合があります。macOS では「プライバシーとセキュリティ → マイク」で ClawX を許可し、完全に終了して再起動してください。開発時は起動元の IDE やターミナルに権限が紐づく場合があり、そちらも再起動が必要です。Windows ではデスクトップアプリのマイクアクセスを許可してください。ダイアログを閉じて再度マイクをクリックすると再試行できます。録音は自動では開始されません。
...
- **📡 マルチチャネル管理**：複数アカウント、アカウント単位のAgent紐付け、既定アカウントの切り替え、Tencent公式個人WeChatチャネルプラグイン、既存の `dingtalk` チャネルにリマップした公式 DingTalk コネクタを備えています。DingTalk の追加時または既存の DingTalk 設定のアップグレード時に、ClawX は同梱の Workspace CLI を構成し、カレンダーやドキュメントなどの `dws` スキル向けに任意の DingTalk Workspace OAuth を案内します。スキップしてもチャットには影響せず、設定済みチャンネルから後で認可を開始または解除できます。組織の主管理者は、事前に DingTalk 開発者コンソールでメンバーによる CLI からの個人データアクセスを有効にする必要があります。
...
- **🔐 セキュアなプロバイダー統合**：OpenAI、Anthropic、Z.AI / GLMなどに接続し、認証情報をOSのネイティブキーチェーンに安全に保存できます。カスタムプロバイダーや互換性フォールバックも利用でき、インターフェース言語が中国語の場合は、PKCEとClawXリクエスト帰属付きのブラウザOAuthに対応するTokenDanceもプロバイダーカタログに表示されます。開発者モードでは **Models → Image Generation** で画像生成エンドポイントを設定できます。生成された画像は、OpenClaw の生の `MEDIA:` パスを表示せず、チャット内に直接表示されます。
